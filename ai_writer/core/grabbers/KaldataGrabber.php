<?php
/**
 * Kaldata Grabber
 * Source: https://www.kaldata.com/it-%d0%bd%d0%be%d0%b2%d0%b8%d0%bd%d0%b8
 */

namespace AiWriter\Core\Grabbers;

use AiWriter\Core\BaseGrabber;

class KaldataGrabber extends BaseGrabber {
    public function getId(): string { return 'kaldata'; }
    public function getName(): string { return 'Kaldata'; }
    public function getUrl(): string { return 'https://www.kaldata.com/it-%d0%bd%d0%be%d0%b2%d0%b8%d0%bd%d0%b8'; }
    public function getDefaultCategory(): string { return 'texnologiya'; }
    public function isRewriteEnabledByDefault(): bool { return true; }
    public function getDefaultRetryMinutes(): int { return 45; }

    public function grab(): array {
        $sourceUrl = $this->getSourceUrl();
        $html = $this->fetchUrl($sourceUrl);
        if (!$html) return [];

        $xpath = $this->parseHtml($html);
        if (!$xpath) return [];

        $items = [];
        $links = $xpath->query("//h2/a | //h3/a | //a[contains(@href, '.html')]");
        $seen = [];

        foreach ($links as $linkNode) {
            $href = $this->nodeAttr($linkNode, 'href');
            if (empty($href)) continue;
            if (!str_contains($href, '/it-') || !str_ends_with(parse_url($href, PHP_URL_PATH) ?? '', '.html')) continue;

            $title = trim($linkNode->textContent);
            if (strlen($title) < 10) continue;

            if (isset($seen[$href])) continue;
            $seen[$href] = true;

            $absUrl = $this->makeAbsoluteUrl($href, $this->getUrl());
            $item = $this->grabArticle($absUrl, $title);
            if ($item) {
                $items[] = $item;
            }

            if (count($items) >= 10) break;
        }

        return $items;
    }

    public function grabArticle(string $url, string $fallbackTitle = ''): ?array {
        $html = $this->fetchUrl($url);
        if (!$html) return null;

        $xpath = $this->parseHtml($html);
        if (!$xpath) return null;

        $title = $this->xpathText($xpath, "//h1") ?: $this->extractMeta($xpath, 'og:title') ?: $fallbackTitle;
        $excerpt = $this->extractMeta($xpath, 'og:description');
        $img = $this->extractMeta($xpath, 'og:image');

        $bodyNodes = $xpath->query(
            "//div[contains(@class, 'entry-content') or contains(@class, 'post-content') or contains(@class, 'article-content')]//p | " .
            "//div[contains(@class, 'entry-content') or contains(@class, 'post-content') or contains(@class, 'article-content')]//h2 | " .
            "//div[contains(@class, 'entry-content') or contains(@class, 'post-content') or contains(@class, 'article-content')]//h3"
        );
        if (!$bodyNodes || $bodyNodes->length === 0) {
            $bodyNodes = $xpath->query("//article//p | //article//h2");
        }

        $paragraphs = [];
        foreach ($bodyNodes as $node) {
            $cls = $node instanceof \DOMElement ? $node->getAttribute('class') : '';
            if (str_contains($cls, 'disclaimer') || str_contains($cls, 'byline') || str_contains($cls, 'sharedaddy')) {
                continue;
            }

            $text = trim($node->textContent);
            if (strlen($text) < 15) continue;
            if (str_contains($text, 'Споделете това:') || str_contains($text, 'Свързани статии:')) continue;

            $tag = $node->nodeName;
            if ($tag === 'h2' || $tag === 'h3') {
                $paragraphs[] = "<{$tag}>" . htmlspecialchars($text) . "</{$tag}>";
            } else {
                $paragraphs[] = "<p>" . htmlspecialchars($text) . "</p>";
            }
        }

        $content = implode("\n", $paragraphs);
        if (empty($content)) {
            $content = "<p>" . htmlspecialchars($excerpt ?: $title) . "</p>";
        }

        return [
            'external_id' => md5($url),
            'url' => $url,
            'title' => $title,
            'excerpt' => $excerpt ?: mb_substr(strip_tags($content), 0, 200),
            'content' => $content,
            'image_url' => $this->makeAbsoluteUrl($img, $url),
            'tags' => ['IT', 'Texnologiya'],
            'category' => $this->getDefaultCategory()
        ];
    }
}
