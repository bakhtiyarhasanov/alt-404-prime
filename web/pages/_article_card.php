...
<article
  ...
  <div class="relative w-full aspect-video overflow-hidden bg-neutral-900">
    <a href="/<?= e($a['category']) ?>/<?= e($a['slug']) ?>" class="block w-full h-full">
      <?php if (!empty($a['image_url'])): ?>
        <img src="<?= e($a['image_url']) ?>" alt="<?= e($a['title']) ?>"
    ...
    <div class="absolute top-2 left-2 z-10 pointer-events-none">
      <span
        class="inline-flex items-center px-2 py-0.5 rounded-xs text-[9px] font-semibold uppercase tracking-wider bg-[#fcdb56] text-[#080117] shadow-xs">
        <?= e($cleanCatLabel) ?>
      </span>
    </div>

    <!-- Bookmark Button -->
    <button type="button" data-bookmark-btn data-bookmark-id="<?= e($a['id']) ?>" data-id="<?= e($a['id']) ?>"
      data-title="<?= e($a['title']) ?>" data-slug="<?= e($a['slug']) ?>" data-category="<?= e($a['category']) ?>"
      data-category-name="<?= e($cleanCatLabel) ?>" data-image="<?= e($a['image_url'] ?? '') ?>"
      ...
      <svg class="w-3.5 h-3.5 bookmark-icon pointer-events-none" viewBox="0 0 24 24" fill="none" stroke="currentColor"
  ...
  <div class="p-3 sm:p-3.5 flex-1 flex flex-col justify-between">
    <div>
      <div
        class="flex items-center gap-2 text-[11px] text-neutral-500 dark:text-neutral-400 mb-1.5 font-medium font-mono">
    ...
    <!-- Footer Meta -->
    <div
      class="mt-3 pt-2.5 border-t border-neutral-100 dark:border-[#22153e] flex items-center justify-between text-[11px] text-neutral-500 dark:text-neutral-400">
      <span class="font-mono text-neutral-400"><?= formatDateAz($a['created_at']) ?></span>
      <a href="/<?= e($a['category']) ?>/<?= e($a['slug']) ?>"
        class="text-[#080117] dark:text-[#fcdb56] font-bold text-[11px] group-hover:translate-x-0.5 transition-transform flex items-center gap-0.5">
        <span>Oxu</span>
        <span>→</span>
      </a>
    </div>
