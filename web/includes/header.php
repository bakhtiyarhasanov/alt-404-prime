...
<head>
  ...
  <link
    href="https://fonts.googleapis.com/css2?family=Tomorrow:ital,wght@0,300;0,400;0,500;0,600;0,700;1,400;1,600&display=swap"
    rel="stylesheet">
  ...
  <!-- Google tag (gtag.js) -->
  <script async src="https://www.googletagmanager.com/gtag/js?id=G-ES6BS4N9PC"></script>
  <script>
    window.dataLayer = window.dataLayer || [];
    function gtag() { dataLayer.push(arguments); }
    gtag('js', new Date());

    gtag('config', 'G-ES6BS4N9PC');
  </script>
</head>

<body
  class="bg-[#FAF8F5] text-neutral-900 dark:bg-[#080117] dark:text-neutral-100 font-sans antialiased selection:bg-[#fcdb56] selection:text-[#080117] transition-colors duration-300 min-h-screen flex flex-col">
  ...
  <div class="sticky top-0 z-40 w-full shadow-xs">
    ...
    <div id="live-news-ticker"
      class="bg-[#080117] text-neutral-100 text-xs py-1.5 px-4 border-b border-[#22153e] flex items-center overflow-hidden">
        ...
        <div class="animate-ticker-marquee flex items-center gap-6 text-[11px]">
          <?php foreach ([$tickerArticles, $tickerArticles] as $loopSet): ?>
            <?php foreach ($loopSet as $tArt): ?>
              ...
              <a href="/<?= e($tArt['category']) ?>/<?= e($tArt['slug']) ?>"
                class="inline-flex items-center gap-2 text-neutral-200 hover:text-[#fcdb56] transition-colors cursor-pointer text-left whitespace-nowrap group/item">
                <span
                  class="px-1.5 py-0.5 rounded-xs bg-[#fcdb56]/20 text-[#fcdb56] font-mono text-[9px] font-bold uppercase border border-[#fcdb56]/30">
    ...
    <header id="main-header"
      class="w-full border-b transition-colors duration-200 backdrop-blur-md bg-white/95 border-neutral-200 text-neutral-900 dark:bg-[#080117]/95 dark:border-[#22153e] dark:text-neutral-100">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-2 flex items-center justify-between gap-2.5 sm:gap-4">
        ...
        <div class="flex items-center shrink-0">
          <a href="/" id="brand-logo-link"
            class="group flex items-center hover:opacity-85 transition-opacity select-none py-1"
            title="ALT404 - Əsas səhifə">
            <img src="/assets/img/logo-dark.png" alt="ALT404"
              class="h-8 sm:h-9 w-auto object-contain select-none dark:hidden" loading="eager">
            <img src="/assets/img/logo.png" alt="ALT404"
              class="h-8 sm:h-9 w-auto object-contain select-none hidden dark:block" loading="eager">
          </a>
        </div>

        <?php if (!empty($topRankedTags)): ?>
          <!-- Left Scroll Button for Hashtags -->
          <button id="scroll-tags-left-btn" type="button"
            ...
            title="Sola sürüşdür" aria-label="Sola sürüşdür">
          ...
          <button id="scroll-tags-right-btn" type="button"
            class="w-9 h-9 hidden md:flex items-center justify-center rounded-lg border transition-all text-xs font-medium cursor-pointer shrink-0 border-neutral-200 bg-neutral-100/90 text-neutral-700 hover:text-black hover:border-neutral-300 hover:bg-neutral-200/80 dark:border-[#2a1a4a] dark:bg-[#130728] dark:text-neutral-200 dark:hover:text-[#fcdb56] dark:hover:border-[#fcdb56]/40 dark:hover:bg-[#1e0d3d]"
            title="Sağa sürüşdür" aria-label="Sağa sürüşdür">
            <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
              stroke-linecap="round" stroke-linejoin="round">
        ...
        <div class="flex items-center gap-2 shrink-0">
          ...
          <button id="open-search-btn" type="button"
            ...
            title="Axtarış (Cmd+K)" aria-label="Axtarış">
            <svg class="w-4 h-4 text-current" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
              stroke-linecap="round" stroke-linejoin="round">
          ...
          <!-- Bookmarks Button -->
          <button id="open-bookmarks-btn" type="button"
            ...
            title="Saxlanılan məqalələr" aria-label="Saxlanılan məqalələr">
            <svg class="w-4 h-4 text-current" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
              stroke-linecap="round" stroke-linejoin="round">
              <path d="M19 21l-7-5-7 5V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2z" />
            </svg>
            <span id="bookmarks-counter-badge"
          ...
          <button id="theme-toggle-btn" type="button"
            ...
            title="Rejimi dəyişdir" aria-label="Rejimi dəyişdir">
            <!-- Sun icon (Light mode) -->
            <svg class="theme-sun-icon w-4 h-4 text-[#080117] dark:hidden" viewBox="0 0 24 24" fill="none"
            ...
            <!-- Moon icon (Dark mode) -->
            <svg class="theme-moon-icon w-4 h-4 text-[#fcdb56] hidden dark:block" viewBox="0 0 24 24" fill="none"
              stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
    ...
    <nav id="category-navigation" aria-label="Əsas kateqoriyalar"
